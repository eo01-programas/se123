(() => {
    ...
    function filterByExactOpPartida(query) {
        ...
        const exactMatches = state.records.filter((r) => getRecordOpPartidaKey(r) === normalizedQuery);
    ...
    // El formulario solo se re-puebla cuando cambia la seleccion o el estado de
    // sus pases: un refresh en segundo plano re-renderiza los resultados y no
    // debe pisar lo que el operario esta escribiendo.
    function computeFormPopulateKey() {
        return Array.from(state.selectedIds).sort().map((id) => {
            const record = findRecordById(id);
            return `${id}:${record && hasOpenPass(record) ? 'abierto' : 'cerrado'}`;
        }).join('|');
    }

    function syncFormWithSelection() {
        const key = computeFormPopulateKey();
        if (key === state.lastFormPopulateKey) return;
        state.lastFormPopulateKey = key;
        populateForm();
    }
    ...
    function search(query) {
        ...
        if (state.currentQuery) scheduleAutoRefreshOnQuery();
        renderResults();
    }
    ...
    // tablet o la PC pudieron actualizar la misma OP-PTDA (p. ej. un INICIO
    // registrado en otro equipo). Si el Sheet ya se consulto hace poco, la
    ...
    function scheduleAutoRefreshOnQuery() {
        if (autoRefreshQueryTimer) clearTimeout(autoRefreshQueryTimer);
